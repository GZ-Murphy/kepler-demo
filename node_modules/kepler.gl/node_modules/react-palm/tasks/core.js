...
function _run(task, fnApplication, success, error) {
...
function taskCreator_(effector, payload, label) {
  ...
  var effectorPrime = function effectorPrime(success, error) {
  ...
  var newTask = _task(payload, function (runEffect, success, error) {
    return runEffect(effectorPrime, success, error);
  }, label);
...
function _task(payload, next, label) {
  ...
  function map(successTransform) {
    return _task(payload, function (runEffect, success, error) {
...
function all(tasks) {
  return _task(tasks.map(function (task) {
    return task.payload;
  }), function (runEffect, success, error) {
    if (tasks.length === 0) {
      return success([]);
    }
    ...
    return Promise.all(tasks.map(function (task, index) {
      return task.run(runEffect, allSuccess(index), anyError);
    }));
  }, 'Task.all(' + tasks.map(function (_ref) {
    var type = _ref.type;
    return type;
  }).join(', ') + ')');
}